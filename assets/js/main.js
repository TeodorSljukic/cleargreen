document.addEventListener("DOMContentLoaded", function() {
  ...
  // ✅ Dropdown klik – radi i u desktop i mobilnom meniju
  const allMenus = document.querySelectorAll('.menu-list li.menu-item-has-children, .mobile-menu-list li.menu-item-has-children');

  allMenus.forEach(li => {
    const link = li.querySelector('a');

    link.addEventListener('click', function(e) {
      // Radi samo na mobilnim uređajima ili u overlayu
      if (window.innerWidth < 900 || overlay.classList.contains('active')) {
        e.preventDefault();

        // Zatvori ostale otvorene dropdownove unutar istog menija
        const siblings = li.parentElement.querySelectorAll('.open');
  ...
  // ✅ Klik na običan link unutar overlay menija zatvara meni
...
/* ✅ Slider (tvoj originalni kod) */
document.addEventListener("DOMContentLoaded", () => {
  const sliders = document.querySelectorAll(".slika-slider");
...
(function ($) {
  ...
  $(window).on("scroll", function () {
    if ($(window).scrollTop() >= 100) {
      ...
      // Promeni logo na sticky verziju
      $(".header-logo").each(function() {
        var stickyLogo = $(this).data("sticky-logo");
        if (stickyLogo) {
          $(this).attr("src", stickyLogo);
        }
      });
    } else {
      $(".header").removeClass("fixed-header");
      // Vrati logo na normalnu verziju
      $(".header-logo").each(function() {
        var normalLogo = $(this).data("normal-logo");
        if (normalLogo) {
          $(this).attr("src", normalLogo);
        }
      });
    }
  });
